...
public:
    ...
    bool SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nPurificationRoundsMin, int nPurificationRoundsMax) const;
    bool SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nPurificationRoundsMin, int nPurificationRoundsMax);
    ...
    std::string PreparePurificationDenominate(int minRounds, int maxRounds);
    int GeneratePurificationOutputs(int nTotalValue, std::vector<CTxOut>& vout);
    ...
    // get the Purification chain depth for a given input
    int GetRealInputPurificationRounds(CTxIn in, int rounds) const;
    // respect current settings
    int GetInputPurificationRounds(CTxIn in) const;
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache = true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++) {
            ...
            const int rounds = pwallet->GetInputPurificationRounds(vin);
            if (rounds >= -2 && rounds < nPurificationRounds) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
                if (!MoneyRange(nCredit))
...
public:
    ...
    //Used with Purification. Will return largest nondenom, then denominations, then very small inputs
    int Priority() const
    {
