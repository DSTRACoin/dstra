...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (fAllFromMeDenom && fAllToMeDenom && nFromMe * nToMe) {
            parts.append(TransactionRecord(hash, nTime, TransactionRecord::PurificationDenominate, "", -nDebit, nCredit));
            ...
            if (mapValue["DS"] == "1") {
                ...
                for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++) {
                    ...
                    if (wallet->IsCollateralAmount(txout.nValue)) sub.type = TransactionRecord::PurificationMakeCollaterals;
                    if (wallet->IsDenominatedAmount(txout.nValue)) sub.type = TransactionRecord::PurificationCreateDenominations;
                    if (nDebit - wtx.GetValueOut() == OBFUSCATION_COLLATERAL) sub.type = TransactionRecord::PurificationCollateralPayment;
                }
            }
