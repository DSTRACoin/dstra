...
DSTRA now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<dstra_address\> \<monthly_payment_dstra\> [use_ix(true|false)]
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<dstra_address\> \<monthly_payment_dstra\> \<collateral_hash\>
 ...
 * vote-many          - Vote on a DSTRA initiative
 * vote-alias         - Vote on a DSTRA initiative
 * vote               - Vote on a DSTRA initiative/budget
