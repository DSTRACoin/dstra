...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem* item, int column)
{
    if (column == COLUMN_CHECKBOX && item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64 characters (this means its a child node, so its not a parent node in tree mode)
    {
        ...
        if (item->checkState(COLUMN_CHECKBOX) == Qt::Unchecked)
            ...
            CTxIn vin(outpt);
            int rounds = pwalletMain->GetInputPurificationRounds(vin);
            if (coinControl->useObfuScation && rounds < nPurificationRounds) {
                QMessageBox::warning(this, windowTitle(),
                    tr("Non-anonymized input selected. <b>Purification will be disabled.</b><br><br>If you still want to use Purification, please deselect all non-nonymized inputs first and then check Purification checkbox again."),
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    if (nQuantity > 0) {
        ...
        // IX Fee
        if (coinControl->useBriskTX) nPayFee = max(nPayFee, CENT);
        // Allow free?
